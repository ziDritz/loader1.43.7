class dofus.graphics.battlefield.EffectsOverHead extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   var _oSprite;
   var _aEffects;
   var _aEffectsQty;
   ...
   function EffectsOverHead(oSprite)
   {
      super();
   ...
   {
      this._oSprite.EffectsManager.addEventListener("effectsChanged",this);
   }
   function initialize()
   {
      this.createEmptyMovieClip("_mcEffects",10);
      this._aEffects = this._oSprite.EffectsManager.getEffects();
      this.drawClip();
   }
   function drawClip()
   {
      ...
      for(var j in this._aEffectsQty)
      {
         ...
         _loc4_._x = _loc5_;
         _loc5_ += dofus.graphics.battlefield.EffectsOverHead.ICON_WIDTH;
