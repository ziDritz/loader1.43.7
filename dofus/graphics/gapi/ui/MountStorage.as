class dofus.graphics.gapi.ui.MountStorage extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function set parkMounts(eaMount)
   {
   ...
   function makeDataProvider(eaSrc, cbFilter)
   {
      ...
      switch(_loc5_)
      {
         ...
         case 3:
            for(var i in eaSrc)
            {
               if(eaSrc[i].fecondable && eaSrc[i].fecondation == -1 || eaSrc[i].fecondable && eaSrc[i].tired == eaSrc[i].tiredMax)
               {
                  _loc4_.push(eaSrc[i]);
               }
            }
