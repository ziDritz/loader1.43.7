class dofus.graphics.gapi.ui.MonsterAndLookSelector extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function filterResult(sFilter)
   {
      ...
      {
         var _loc6_ = new ank.utils.ExtendedString(_loc3_[_loc5_].name).removeAccents().toUpperCase();
         if(!(sFilter.length && (sFilter != "Recherche..." && (sFilter.length && _loc6_.indexOf(sFilter) == -1))))
         {
   ...
   function click(oEvent)
   {
      switch(oEvent.target._name)
      {
         ...
         case "_btnCancel":
            ...
            this.callClose();
         case "_btnSelect":
            this.select({target:{contentData:this._cg.selectedContentData}});
            break;
         case "_btnSearch":
            this._cbType.selectedIndex = 0;
            this.filterResult(new ank.utils.ExtendedString(this._tiSearch.text).removeAccents().toUpperCase());
      }
   }
