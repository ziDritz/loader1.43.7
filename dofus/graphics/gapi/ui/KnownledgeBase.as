class dofus.graphics.gapi.ui.KnownledgeBase extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function set article(nArticleID)
   {
   ...
   function generateIndexes()
   {
      ...
      while(_loc2_ < this._eaArticles.length)
      {
         ...
         {
            this._eaIndexes.push({name:new ank.utils.ExtendedString(this._eaArticles[_loc2_].k[_loc3_]).removeAccents().toUpperCase(),i:this._eaArticles[_loc2_].i});
            _loc3_ = _loc3_ + 1;
         }
         _loc2_ = _loc2_ + 1;
      }
   ...
   {
      var _loc0_ = null;
      if((_loc0_ = oEvent.target._name) === "_tiSearch")
      {
         var _loc3_ = this._tiSearch.text;
         if(_loc3_.length > 0)
         {
            this.switchToDisplay(dofus.graphics.gapi.ui.KnownledgeBase.DISPLAY_SEARCH);
            this.searchTopic(new ank.utils.ExtendedString(_loc3_).removeAccents().toUpperCase());
         }
         else
         {
            this.switchToDisplay(dofus.graphics.gapi.ui.KnownledgeBase.DISPLAY_CATEGORIES);
   ...
   {
      var _loc0_ = null;
      if((_loc0_ = sShortcut) === "ACCEPT_CURRENT_DIALOG")
      {
