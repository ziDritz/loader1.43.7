class dofus.aks.Mount extends dofus.aks.Handler
{
   ...
   function onMountPark(sExtraData)
   {
      ...
      while(_loc5_ < _loc3_.length)
      {
         ...
         var _loc13_ = this.api.kernel.CharactersManager.createGuildEmblem(_loc12_);
         var _loc14_ = Number(_loc6_[6]);
         var _loc15_ = new dofus.datacenter.MountPark(_loc7_,_loc8_,_loc9_,undefined,_loc10_,_loc11_,_loc13_,this.api.datacenter.Map.id,_loc4_,_loc14_);
   ...
   {
      var _loc3_ = sExtraData.split("|");
      this.api.ui.loadUIComponent("MountParkSale","MountParkSale",{defaultPrice:Number(_loc3_[1]),instance:Number(_loc3_[2])});
   }
   function onLeave()
   {
