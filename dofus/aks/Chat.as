class dofus.aks.Chat extends dofus.aks.Handler
{
   ...
   function onMessage(bSuccess, sExtraData)
   {
      ...
      switch(_loc4_)
      {
         ...
         case "e":
            ...
            var _loc24_ = _loc10_.split(";");
            _loc6_ = "EVENT_" + String(_loc24_[0]) + "," + _loc24_[1];
            break;
         case "@":
            _loc12_ = "ADMIN_CHAT";
            var _loc25_ = "(" + this.api.lang.getText("PRIVATE_CHANNEL") + ") " + _loc7_ + " : ";
            _loc6_ = "(" + this.api.lang.getText("PRIVATE_CHANNEL") + ") " + this.getLinkName(_loc8_,_loc7_) + " : " + this.getLinkMessage(_loc8_,_loc7_,_loc25_,_loc10_,_loc6_);
            break;
         default:
            var _loc26_ = this.isLivingItemMessage(_loc6_);
            if(this.api.lang.getConfigText("EMOTES_ENABLED") && (!_loc26_ && (_loc6_.charAt(0) == dofus.Constants.EMOTE_CHAR && _loc6_.charAt(_loc6_.length - 1) == dofus.Constants.EMOTE_CHAR)))
            {
               if(!this.api.datacenter.Game.isRunning && this.api.kernel.ChatManager.isTypeVisible(2))
               {
                  var _loc27_ = !(_loc6_.charAt(_loc6_.length - 2) == "." && _loc6_.charAt(_loc6_.length - 3) != ".") ? _loc6_ : _loc6_.substr(0,_loc6_.length - 2) + dofus.Constants.EMOTE_CHAR;
                  _loc27_ = dofus.Constants.EMOTE_CHAR + _loc27_.charAt(1).toUpperCase() + _loc27_.substr(2);
                  this.api.gfx.addSpriteBubble(_loc8_,this.api.kernel.ChatManager.applyInputCensorship(_loc27_));
               }
               _loc12_ = "EMOTE_CHAT";
            ...
            else if(_loc6_.substr(0,7) == "!THINK!")
            {
               ...
               _loc12_ = "THINK_CHAT";
               var _loc28_ = _loc7_ + " " + this.api.lang.getText("THINKS_WORD") + " : ";
               _loc6_ = "<i>" + this.getLinkName(_loc8_,_loc7_) + " " + this.api.lang.getText("THINKS_WORD") + " : " + this.getLinkMessage(_loc8_,_loc7_,_loc28_,_loc10_,_loc6_) + "</i>";
            }
            else if(_loc26_ && !_global.isNaN(_loc6_.substr(2,_loc6_.length - 4)))
            {
            ...
            else
            {
               ...
               _loc12_ = "MESSAGE_CHAT";
               var _loc29_ = _loc7_ + " : ";
               _loc6_ = this.getLinkName(_loc8_,_loc7_) + " : " + this.getLinkMessage(_loc8_,_loc7_,_loc29_,_loc10_,_loc6_);
               if(this.api.datacenter.Player.isAuthorized)
               {
                  var _loc30_ = this.api.kernel.DebugManager.getTimestamp();
                  this.api.kernel.ChatManager.addRawMessage(this.api.datacenter.Map.id,_loc12_,this.getRawFullMessage(_loc29_,_loc10_),_loc30_);
               }
            }
      }
   ...
   function parseLivingItemMessage(sMessage, sFromID, sFromName, sRawMessage)
   {
      ...
      if(_loc7_.m)
      {
         ...
         return this.getLinkName(sFromID,sFromName,true) + " : " + this.getLinkMessage(sFromID,sFromName,_loc8_,sRawMessage,sMessage);
      }
      return undefined;
   }
}
